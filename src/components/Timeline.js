    ...
    <section id="timeline" className="text-white py-20 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col gap-6 mb-8">
          <h2 className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#E61933] to-[#FF33CC] font-['Inter']">Timeline</h2>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-8">
          {schedule.map((day, index) => (
            <div key={index} className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6 relative overflow-hidden">
               ...
               <div className="absolute top-[20%] -left-5 w-[100px] h-[200px] bg-gradient-to-r from-[#E61933] to-transparent opacity-10 rotate-[15deg] blur-[20px] pointer-events-none"></div>

              <div className="mb-6 border-b border-white/10 pb-4">
                <h3 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#E61933] to-[#FF33CC] font-['Inter']">{day.day}</h3>
              </div>
              <div className="flex flex-col gap-6">
                {day.events.map((event, idx) => (
                  <div key={idx} className="flex flex-col gap-1">
                    <span className={`text-sm ${event.highlight ? 'text-[#E61933] font-bold' : 'text-[#a3a3a3] font-normal'}`}>{event.time}</span>
